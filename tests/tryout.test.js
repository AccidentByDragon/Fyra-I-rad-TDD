...
test('1: Does the logoheadline have the text: Four in a Row?', () => {
  ...
  new App();
  expect((body.querySelector('h1')).innerText).toBe('Fyra-I-Rad');
});
...
test('7: Does the game app prevent placing a disc when a column is full?', async () => {

  let body = await registerPlayers();
  let as = async () => await sleep(400);

  // Fill up the first column (column 1)
  click(body.querySelector('.cell:nth-child(1)')); await as();
  expect(body.querySelector('.cell:nth-child(36)').classList.contains('Red')).toBeTruthy();
  ...
  click(body.querySelector('.cell:nth-child(1)')); await as();
  expect(body.querySelector('.cell:nth-child(22)').classList.contains('Red')).toBeTruthy();

  click(body.querySelector('.cell:nth-child(1)')); await as();
  expect(body.querySelector('.cell:nth-child(15)').classList.contains('Yellow')).toBeTruthy();
  ...
  // Attempt to place another disc in the full column (column 1)
  click(body.querySelector('.cell:nth-child(1)')); await as();

  // The first cell should still be "Yellow" (unchanged) since no disc can be placed
  expect(body.querySelector('.cell:nth-child(1)').classList.contains('Yellow')).toBeTruthy();

  // Ensure no new disc is placed in a full column
  let cellsInColumn1 = [
    body.querySelector('.cell:nth-child(1)').classList,
    body.querySelector('.cell:nth-child(8)').classList,
    body.querySelector('.cell:nth-child(15)').classList,
    body.querySelector('.cell:nth-child(22)').classList,
    body.querySelector('.cell:nth-child(29)').classList,
    body.querySelector('.cell:nth-child(36)').classList
  ];

  // Check if all cells in the column are filled with no change after the final click
  expect(cellsInColumn1[0].contains('Yellow')).toBeTruthy();
  expect(cellsInColumn1[1].contains('Red')).toBeTruthy();
  expect(cellsInColumn1[2].contains('Yellow')).toBeTruthy();
  expect(cellsInColumn1[3].contains('Red')).toBeTruthy();
  expect(cellsInColumn1[4].contains('Yellow')).toBeTruthy();
  expect(cellsInColumn1[5].contains('Red')).toBeTruthy();

}, 10000);


test('8: Prevent placing a disc after game is won or tied', async () => {

  let body = await registerPlayers();
  let as = async () => await sleep(400);

  // Simulate winning
  click(body.querySelector('.cell:nth-child(1)')); await as(); // Red
  click(body.querySelector('.cell:nth-child(2)')); await as(); // Yellow
  click(body.querySelector('.cell:nth-child(1)')); await as(); // Red
  click(body.querySelector('.cell:nth-child(2)')); await as(); // Yellow
  click(body.querySelector('.cell:nth-child(1)')); await as(); // Red
  click(body.querySelector('.cell:nth-child(2)')); await as(); // Yellow
  click(body.querySelector('.cell:nth-child(1)')); await as(); // Red (winning move)

  // The game should now be in a "won" state
  console.log('Game won: ', body.querySelector('.cell:nth-child(1)').classList);

  // Expect no more discs can be placed
  click(body.querySelector('.cell:nth-child(1)')); await as(); // Red trying to place again

  expect(body.querySelector('.cell:nth-child(1)').classList.contains('Red')).toBeTruthy();

  // Log and check the game over message
  console.log('Game Over Message: ', body.querySelector('.game-over-message').textContent);
  expect(body.querySelector('.game-over-message').textContent).toContain('Red wins!');
